	...
	*/
	public function SEO_IMAGES_wpdocs_metabox_callback( $post ){
	    ...
	    <div class="seo_images-metabox-cvr">
	    	...
	    	<p class="notice-info notice-info-gutenberg" style="display: none;"><span class="red-sp">Note:</span>Please convert article to Gutenberg Blocks by clicking Editor Classic banner below article title.</p>
	    	...
	    	<div class="seo_images-group"> 	
	    		...
	    		<div class="seo_images-inner-holder">
	    			<div class="seo_images-inner-pad hide_if_guten">
		...
		if( !empty($_POST) ) {
            ...
            if( $rowData->status == 0 ) {
                ...
                die(json_encode(array(
                    "error" => "account_validation_pending",
                    "error_msg" => $rowData->message
                )));
            } else {
    ...
    function insert_selected_imageApi() {
        ...
        $image_size = sanitize_text_field( $_POST['image_size'] );
        $image_align = sanitize_text_field( $_POST['image_align'] );
        //$show_image_author = get_option( 'seo_images_show_author' );
        //$show_author = $show_image_author == '1' ? 'show_author' : '';
        $show_author = sanitize_text_field( $_POST['show_author'] );
        ...
        foreach ( $_POST['images'] as $image) {
            ...
            if( !empty( $image[0] ) && !empty( $image[1] ) ) {
                ...
                if($uploaded_path != 'no_image' && !empty($uploaded_path)) {
                    ...
                    $author = isset( $explode_author[2] ) ? 'image by: ' . $explode_author[2] : '';
                    ...
                    // Title & Alt with AI
                    $api_key = get_option( 'sc_api_key' );
                    $title_response = wp_remote_get( WP_SEO_PLUGINS_BACKEND_URL . 'chatgpt?api-key=' . $api_key  . '&image=' . $upl_path . '&heading=' . trim( $heading_val[$i] ), array(
                        'timeout'     => 30,
                        'sslverify' => false
                    ));
                    if( !is_wp_error( $title_response ) ){
                        $title_obj = json_decode( $title_response['body'] );
                        if( $title_obj->status == 0 ){
                            $title = $title_obj->title;
                        }
                    }

                    $alt_response = wp_remote_get( WP_SEO_PLUGINS_BACKEND_URL . 'chatgpt/alt?api-key=' . $api_key  . '&image=' . $upl_path . '&heading=' . trim( $heading_val[$i] ), array(
                        'timeout'     => 30,
                        'sslverify' => false
                    ));
                    if( !is_wp_error( $alt_response ) ){
                        $alt_obj = json_decode( $alt_response['body'] );
                        if( $alt_obj->status == 0 ){
                            $alt = $alt_obj->alt;
                        }
                    }

                    // End Title & Alt with AI
                }
                ...
                switch( $image_size ){
                    ...
                    case 'full' :
                        $size = 'size-full';
                        $size_width = $upl_width;
                        $size_height = $upl_height;
                        break;
                }
                ...
                switch( $image_align ){
                    ...
                    case 'center':
                        ...
                        break;
                    case 'random':
                        $align = $random_align[0];
                        break;
                }
                $IMG_TAG = '[caption id="attachment_' . $upl_id . '" align="' . $align . '" width="' . $size_width . '"]<img src="' . $upl_path . '" alt=' . stripcslashes( $alt ) . ' title=' . stripcslashes( $title ) . ' width="' . $size_width . '" height="' . $size_height . '" class="' . $size . ' wp-image-6" data-caption="' . $author . '" /> ' . $author . '[/caption]';

                $response_html[$i]['heading_title'] = trim( $heading_val[$i] );
                $response_html[$i]['heading_id'] = trim( $heading_id_ar[$i] );
                $response_html[$i]['heading_image'] = $IMG_TAG;
                $response_html[$i]['uploaded_path_test'] = $uploaded_path;
                $response_html[$i]['title_response'] = $title_response;
                $response_html[$i]['remote_get'] = WP_SEO_PLUGINS_BACKEND_URL . 'chatgpt?api-key=' . $api_key  . '&image=' . $upl_path . '&heading=' . trim( $heading_val[$i] );
            }
            $i++;
        }
